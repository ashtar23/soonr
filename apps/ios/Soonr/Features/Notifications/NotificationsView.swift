...
private struct NotificationsList: View {
    ...
    private var list: some View {
        List {
            if groupsByGame {
                ForEach(NotificationGameGroup.groups(for: records)) { group in
                    groupRow(group, in: records)
                }
            } else {
                ForEach(records) { record in
                    row(record, in: records)
                }
            }
    ...
    private func groupRow(
        ...
        if group.isCollapsed {
            ...
            .hidingOuterSeparators(
                isFirst: group.latest.id == records.first?.id,
                isLast: group.latest.id == records.last?.id
            )
